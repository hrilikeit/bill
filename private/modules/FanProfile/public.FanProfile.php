...
//							new StaysailContainer('B', 'banner', /*$subscription->getClubList() .*/ $banner->getHTML()),
    ...
    public function updateBioForm()
    {
		...
		$writer = new StaysailWriter('edit-profile');
...
//		$writer->draw($bio);
        ...
        $writer->addHTML('<div id="blockButtons">');
        ...
        if ($is_deletion_request) {
            $writer->addHTML('<div style="font-size: 11px; display: block;" class="delete-btn">Account waiting for deletion</div>');
        } else {
            $writer->addHTML('<a style="display: block;" id="myBtn_delete" href="#" class="delete_acc delete-btn">delete account</a>');
        }

        $writer->addHTML('</div></div>');
