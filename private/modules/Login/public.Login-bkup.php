...
class Login extends StaysailPublic
{
    ...
    private function joinScreen()
    {
    	...
    	if ($type != 'Entertainer' and $type != 'Club_Admin') {
    		...
    		$writer->h1('Step 1');
    		$writer->p("<strong>This is the sign-up screen for fans.</strong> If you are an Entertainer or a Club representative, {$other_link}.");
    	}
...
__END__;
    ...
    private function forgotPassword()
    {
		...
		$writer->h1('Forgot Password')
			   ->p("We're sorry to hear that you've forgotten your password.  Please enter your email address below, 
			   	    and we'll send you a link which will allow you to reset your password.")
			   ->draw($request);
		return $writer;
    }
...
__END__;
			...
			mail($email, 'Password Reset', $message, $from);
   			...
   			$writer->h1('Thank you!  Please check your email for further instructions');
   			$writer->draw($this->loginScreen());
    	} else {
    		$writer->h1('Sorry...');
...
__END__;
    ...
    private function entertainerSignupInfoScreen()
    {
    	...
    	$writer->h1('Entertainer Inquiry');
    	$writer->p('We appreciate your interest in Local Strip Fan!  So that we may contact you with information about joining, please fill out the information below.');
    ...
    private function clubSignupInfoScreen()
    {
    	...
    	$form->setSubmit('Send My Information')
    		 ...
    		 ->addField(StaysailForm::Hidden, 'Type', 'type')
       		 ->addField(StaysailForm::Line, 'Your Name', 'name', 'required')
       		 ->addField(StaysailForm::Line, 'Your Role (manager, owner, etc.)', 'role', 'required')
    		 ->addField(StaysailForm::Line, 'Contact Phone Number', 'phone', 'required')
    		 ->addField(StaysailForm::Line, 'Email', 'email', 'required email')
    		 ->addField(StaysailForm::Line, 'Club Name', 'club_name', 'required')
    		 ->addField(StaysailForm::Line, 'Club Address', 'club_address', 'required-if:type')
    		 ->addField(StaysailForm::Line, 'Club Phone Number', 'club_phone', 'required')
			 ->addField(StaysailForm::Line, 'How did you hear about us? (website, club, other person)', 'source');
