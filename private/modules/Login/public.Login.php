...
//                $writer = $this->createGoogleLogin();
            ...
            case 'join-twitter':
                $writer = $this->checkTwitterAccount();
                break;

            case 'create-twitter':
                $type = StaysailIO::get('type');
                $writer = $this->createTwitterLogin($type);
                break;
...
//            }
    ...
    private function loginScreen()
    {
        ...
        $login->setPostMethod()
            ...
            ->addField(StaysailForm::Password, 'Your Password', 'password', 'required')
            ->setSubmit('Log In');
    ...
    private function createGoogleLogin($type)
    {
      ...
      //  var_dump($_SESSION["google_user"]);
        if (!empty($_SESSION["google_user"])) {
            ...
            $name = mb_convert_encoding($userData['name'], 'ISO-8859-1', 'UTF-8');

            $filters = array(new Filter(Filter::Match, array('name' => $name)));
...
__END__;
    ...
    private function checkTwitterAccount()
    {

        if (!empty($_SESSION["twitter_user"])) {
           ...
           // $userData['id'] = "123";
            ...
            $filters = array(new Filter(Filter::Match, array('twitter_id' => $userData['id'])));
            $Previous_Member_With_Twitter_id = $this->framework->getSingle('Member', $filters);

            if ($Previous_Member_With_Twitter_id) {
                $this->loginUser($Previous_Member_With_Twitter_id);
            }
            else{
                header("Location:/index.php?mode=Login&job=join&showModal=1");
//                header("Location:/index.php?mode=Login&job=create-google");
    ...
    private function createTwitterLogin($type)
    {
        if (!empty($_SESSION["twitter_user"])) {
            $userData = $_SESSION["twitter_user"];

//            $userData['email'] = "bill@localstripfan.com";
//            $userData['name'] = "123";
            ...
            <div id='myModalGoogle' class='modalGoogle $showModal'>
              ...
              <div class='modal-header-google'>
                ...
                <div class='modal_box'>
                    <div id='adbox'>
                        <div class='adbox1'>
                            <a href='?mode=Login&job=create-google&type=Fan'>FAN</a>
                        </div>
                        <div class='adbox2'>
            ...
            <div id='myModalGoogle' class='modalGoogle $showModal'>
              ...
              <div class='modal-header-google'>
                ...
                <div class='modal_box'>
                    <div id='adbox'>
                        <div class='adbox1'>
                            <a href='?mode=Login&job=create-twitter&type=Fan'>FAN</a>
                        </div>
                        <div class='adbox2'>
                            <a href='?mode=Login&job=create-twitter&type=Entertainer'>CREATOR</a>
                        </div>
                    </div>
                </div>
              </div>
...
//    <div class="switch_container">
//        <label>
//            <input type="checkbox" class="checkbox_fan">
//            <span>FAN</span>
//        </label>
//        <label>
//            <input type="checkbox" class="switch_account">
...
//');
        ...
        $writer->addHTML('
    <div class="switch_container checkbox_container">
    <label>I AM A</label>
        <label>
            <input type="checkbox" class="checkbox_fan">
            <span>FAN</span>
        ...
        <label>
            <input type="checkbox" class="checkbox_creator">
            <span>CREATOR</span>
        </label>
    </div>
');
        ...
        if ($type != 'Entertainer' and $type != 'Club_Admin') {
            ...
            $writer->p("This is for FAN sign up only. If you are a Creator put another check mark on top.");
        }
...
//    	       ->addField(StaysailForm::Select, 'Cell Provider', 'cell_provider', 'required', $providers)
            ...
            ->addHTML('<img style="width: 50%!important; margin-left: 20px;" src = "captcha.php" />')
            ->addField(StaysailForm::Line, '','captcha','required','','','','','Enter captcha here');
...
//        }
       ...
       // $writer->addHTML("<a href='" . $client->createAuthUrl() . "' class='google_btn social_btn'>Google Login</a>");
        ...
        $writer->addHTML('<p>If you have an account,<b> please Sign In </b></p>');
...
//        $writer->span($club_link);
    ...
    {
        $writer = new StaysailWriter(__CLASS__);
        $captcha = StaysailIO::post('captcha');

        if($captcha != $_SESSION['rand_code']){
            $writer->h1("You entered incorrect captcha", 'red');
            return $writer;
        }
...
//        }
        ...
        $name = mb_convert_encoding(StaysailIO::post('name'), 'ISO-8859-1', 'UTF-8');
        $filters = array(new Filter(Filter::Match, array('name' => $name)));
        $Previous_Member_With_Name = $this->framework->getSingle('Member', $filters);
...
//        $fields = array('email', 'phone', 'sms_optout','referral_email', 'created_at');
//
//        $Member->first_name = mb_convert_encoding(StaysailIO::post('first_name'), 'ISO-8859-1', 'UTF-8');;
//        $Member->last_name = mb_convert_encoding(StaysailIO::post('last_name'), 'ISO-8859-1', 'UTF-8');;
//        $Member->name = $name;
...
//                        }
                        ...
                        if ($_SERVER['REMOTE_ADDR'] != '127.0.0.1') {
                            $MailSend->send('support@yourfanslive.com',"New Entertainer: {$account->name}",$message);
//                            mail('support@yourfanslive.com', "New Entertainer: {$account->name}", $message);
                        } else {
                            $MailSend->send('jjustian@gmail.com',"New LOCALHOST Entertainer: {$account->name}",$message);
//                            mail('jjustian@gmail.com', "New LOCALHOST Entertainer: {$account->name}", $message);
                        }
...
//                    die();
                    ...
                    $activation_link = "https://$domain/?mode=PublicLive&job=activate&email=" . urlencode($email);
                    $subject = 'Verification';
                    $message = "<html><head><title>Account Activation</title></head><body><p>Hi,</p><p>Please click the following link to activate your account: <a href='$activation_link'>here</a></p></body></html>";
    ...
    private function saveProfile()
    {
       ...
       // if(!$account_type || !StaysailIO::session('Member.id') || StaysailIO::session('Member.id') == 0){
...
<body>
    <p>Greetings,</p>
    <p>Thank you for your password reset request. To reset your password, please click on the link below or paste the following URL into your web browser's address bar:</p>
    <p><a href="$reset_link">CLICK HERE</a></p>
    <p>You will be asked to change your password on this screen, and you will be able to log in immediately with your new password.</p>
    <p>Please note that this link will expire in a couple of hours. If it does not work, please click the "Forgot Password" link again.</p>
    <p>If you did not request this password change, then disregard this message.</p>
    <p>Regards,</p>
    <p>Local City Scene Management</p>
</body>
</html>
__END__;
            ...
            $MailSend = new MailSend($Member);
            $MailSend->send($email, $subject, $message, 1, 0); // Ensure that send method handles HTML emails
            $writer->addHTML('<div class="box-form">');
            $writer->h1('Thank you!  Please check your email for further instructions! ', 'join');
            $writer->h1('&nbsp', 'join');
            $writer->h1('If you have not received an email please check your SPAM folder or write us an email to authorization@yourfanslive.com','join');
            $writer->h1('&nbsp', 'join');
            $writer->draw($this->loginScreen());
        } else {
            $writer->addHTML('<div class="box-form">');
...
margin-left: 20px;
        ...
        <div class="left-logo">
            ...
            <a href="/">
                <img src="/site_img/FullLogo.png" class="login_img_logo">
            </a>
            <div id="logoSeparator"></div>
             <a href="/">
                <img src="/site_img/logo_95_img.png" class="login_img_logo_95">
            </a>
