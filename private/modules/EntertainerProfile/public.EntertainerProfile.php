...
//                }elseif (!empty($text) && empty($postPhoto['name']) && empty($postVideo['name'])){
            ...
            case 'delete_acc':
               ...
               $content = StaysailIO::post('messageBody');

                $MailSend = new MailSend($this->Member);
                $mail = 'support@yourfanslive.com';
                $MailSend->send($mail, 'Request for delete from ' . $this->Member->name, $content);
                ...
                $html = '<div class="main-div">
                            <div class="profile-top-section">
                                ...
                                <div class="summary-div">
                                    <div class="summary entertainer-summary">
                                        ' . $summary->getHTML() .
...
//            }
        ...
        $writer->addHTML(
            '<div class="modal" id="myModal_delete">
                    <div style="margin: 250px auto; padding: 20px; height: unset;" class="modal-content">
                        <div class="modal-header">
                            <h2>Request for delete profile</h2>
                        </div>
                       <form id="textForm">
                            <label for="textInput">Please describe the reason why you decided to delete your profile.</label>
                            <textarea style="margin: 10px; width: 88%" id="messageBody" name="messageBody" rows="5" cols="100" required></textarea>
                            <div id="requestModalFooter" style="display: flex;">
                                <button type="submit" id="submitRequest" style="color: #FFF; background-color: red;">Submit request</button>
                                <button style="width: 100px; margin-left: 6px;display: block;" type="button" class="close_delete ">close</button>
                            </div>
                        </form>
                    </div>
                  </div>');
...
//            $invitationLink = 'https://'.$_SERVER["SERVER_NAME"]. '/' .$fanUrl ;
            $invitationLink = 'https://'.$_SERVER["SERVER_NAME"]. '/index.php?mode=EntertainerProfile&entertainer_id=' .$this->Entertainer->id;
        }
        if ($plans == 0){
        ...
        $bio->setSubmit('Update Profile')
            ...
            ->addHTML('<div class="label" id="intLink">Your Invitation Link</div><p>' . $invitationLink . '</p>')
...
//            ->addField(StaysailForm::Boolean, 'Do not send text messages', 'sms_optout')
        ...
        */
        ...
        if ($is_deletion_request) {
            ...
            $writer->addHTML('<a id="myBtn_delete" class="delete_acc">delete account</a>');
        }
        return $writer->getHTML();
    }
