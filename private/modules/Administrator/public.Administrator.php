...
class Administrator extends StaysailPublic
{
    ...
    public function __construct($dbc = '')
    {
        ...
        $this->menu = array(
            ...
            'Members:deletion_requests' => 'Deletion requests',
...
__END__;
        ...
        switch ($job) {
            ...
            case 'deletion_requests':
                $content = $this->viewDeletionRequests();
                break;
            case 'accept_delete_request':
                $content = $this->acceptDeleteRequest($id);
                break;
            case 'decline_delete_request':
                $content = $this->declineDeleteRequest($id);
                break;
...
__END__;
    ...
    private function viewDeletionRequests()
    {
//        if (!$this->Admin->has($account)) {
//            return 'No Access!';
//        }
        $writer = new StaysailWriter();
        $writer->h1("Deletion requests");
        $writer->p("<form>Find: <input type=\"text\" name=\"q\" id=\"q\" onfocus=\"startAdminSearch()\" /></form>");
        $table = new StaysailTable('admin');
        $table->setColumnHeaders(['id', 'member', 'request_data']);
        $sql_deletion_requests = "SELECT * FROM `deletion_requests` WHERE `status` = 1";
        $arrayResult = $this->_framework->get_array($sql_deletion_requests);
        foreach ($arrayResult as $request_row) {
            $Member = new Member($request_row["member_id"]);
            $row = array($request_row["id"],
                $Member->name,
                $request_row["created_at"],
                '<a href="#" onclick="modalMail();">Accept</a>',
                StaysailWriter::makeJobLink('Accept', __CLASS__, 'accept_delete_request', $Member->id, '', 'Are you sure?'),
                StaysailWriter::makeJobLink('Decline', __CLASS__, 'decline_delete_request', $Member->id, '', 'Are you sure?'),
            );
...
//            new Filter(Filter::StringCompare, array('created_at', '>=', $dateLimit)),
//            new Filter(Filter::Where, "created_at <= '$dateLimit'"),
            new Filter(Filter::Sort, $account == 'entertainer' ? 'order_list ASC' : 'id DESC')
        );
//        $collection = $this->_framework->getSubset($class, $filters);
        $collection = $this->_framework->getSubsetPerPeriod($class);
...
//                    mail($email, $name, $content . $optout, "Content-Type:text/html\nReply-to:{$reply}\nFrom:{$reply}");
        ...
        if ($handle = opendir('../private/reports')) {
            while (false !== ($entry = readdir($handle))) {
                if (preg_match('/report\.(.+).php/', $entry, $m)) {
                    ...
                    $csv_link = StaysailWriter::makeJobLink('CSV', __CLASS__, 'create_csv', $classname);
    ...
    private function createCsv($classname)
    {
        StaysailIO::cleanse($classname, StaysailIO::Filename);
        $filepath = "../private/reports/report.{$classname}.php";
        if (file_exists($filepath)) {
...
";
    ...
    private function acceptDeleteRequest($id)
    {
        $this->deleteMember($id);
        $this->changeDeletionRequest($id, 2);

        $Member = new Member($id);
        $MailSend = new MailSend($Member);
        $content = "Congratulations! You account deleted";
        $MailSend->send($Member->email, $Member->name, $content);
        return header("Location:?mode=Administrator&job=deletion_requests");
    }

    private function declineDeleteRequest($id)
    {
        $this->changeDeletionRequest($id, 3);
        return header("Location:?mode=Administrator&job=deletion_requests");
    }

    private function changeDeletionRequest($id, $status)
    {
        $sql = "UPDATE `deletion_requests`
					SET admin_id = {$this->Admin->id},
					    status = {$status}
					WHERE member_id = {$id}
					AND status = 1";
