...
{
	if (file_exists(DATAROOT . "/private/avatars/avatar{$avatar_member_id}.{$filetype}")) {
		if ($web) {
			print toBrowserResized($avatar_member_id, $filetype);
		} else {
			header("Content-Type:image/{$filetype}");
			$image = file_get_contents(DATAROOT . "/private/avatars/avatar{$avatar_member_id}.{$filetype}");
			print $image;
		}
		exit;
	}
...
{
	$path = DATAROOT . "/private/avatars/avatar{$avatar_member_id}.{$filetype}";
	$new_width = 360;
