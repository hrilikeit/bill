...
{
	if (file_exists(DATAROOT . "/private/avatars/entertainerDisplayPhoto{$avatar_member_id}.{$filetype}")) {
		//if ($web) {
			//print toBrowserResized($avatar_member_id, $filetype);
		//} else {
			header("Content-Type:image/{$filetype}");
			$image = file_get_contents(DATAROOT . "/private/avatars/entertainerDisplayPhoto{$avatar_member_id}.{$filetype}");
			print $image;
		//}
		exit;
	}
...
$image = file_get_contents(DATAROOT . "/public/site_img/landscape-profile.jpeg");
...
{
	$path = DATAROOT . "/private/avatars/entertainerDisplayPhoto{$avatar_member_id}.{$filetype}";
	$new_width = 800;
